...
volume = 1.0
volume_factor = [.9, 1.7]
add_note(solo, bass, blues_scale[curr_note], 1.0, beats_per_minute, volume)

licks = [ [ [1,0.5*1.1], [-1,0.5], [-1, 0.5*1.1], [2, 0.5], [1,0.5*.9], [-2, .5*.9], [2,.5],[3,1],[-2, .5] ] ]
for i in range(4):
    ...
    for note in lick:
        ...
        add_note(solo, bass, blues_scale[curr_note], note[1], beats_per_minute, volume * random.choice(volume_factor))
